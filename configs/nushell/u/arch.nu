...
export def dev [] {
	run-external ...$PACMAN stylua lua-language-server svelte-language-server typescript typescript-language-server typescript-svelte-plugin pnpm nodejs vscode-css-languageserver vscode-html-languageserver vscode-json-languageserver astyle pacman-contrib actionlint act namcap rustup rust-analyzer
...
export def hyprland [] {
	...
	if $env.HOSTNAME? == 'armpipa-alx' {
		run-external ...$PACMAN vulkan-freedreno vulkan-tools
	}
