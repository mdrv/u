...
def run-warmup []: nothing -> nothing {
	mut $tries = 0
	while not ($MPV_SOCK | path exists) and $tries < 10 {
		sleep 100ms
		$tries = $tries + 1
	}

	if $tries >= 100 {
		error make {msg: "mpv socket not created after 10s"}
	}

	# Device init buffer
	sleep 500ms

	# Warm-up sequence: Play muted for 1s, then reset and unmute
	let $commands = [
		# Start playing (still muted) - warm up device with audio data
		{command: ["set_property" "pause" "no"]}
		# Wait 1sec for warm-up playback (handled below with sleep)
		# Stop playback
		{command: ["set_property" "pause" "yes"]}
		# Seek to 0 (reset to beginning)
		{command: ["seek" "0" "absolute"]}
		# Unmute
		{command: ["set_property" "mute" "no"]}
	]

	# Send commands sequentially with 1sec pause between start and stop
	let $commands_len = ($commands | length)
	for $idx in 0..($commands_len - 1) {
		let $cmd = $commands | get $idx

		# Send the command
		($cmd | to json --raw) + (char nl) | ^socat - $MPV_SOCK | complete

		# If this is the "start playing" command, wait 1sec
		# This allows device to warm up with real audio data
		if $idx == 0 {
			sleep 1sec
		}
	}

	# print "[ mpv.nu] Warm-up commands sent"
}
...
export def main [
	...
	run-external mpv ...$mods ...$add ...$files
}
