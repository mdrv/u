<script lang="ts">
  ...
  function filterTree(nodes: TreeNode[], onlyAnnotated: boolean): TreeNode[] {
    ...
    return nodes
      .map((node) => {
        if (node.type === 'file') {
          return node.isAnnotated || node.path.includes('README.md') ? node : null
  ...
  function getCategory(category: string): string {
    ...
    if (category.includes('kitty') || category.includes('foot') || category.includes('zellij')) return 'terminal'
    if (category.includes('hypr')) return 'compositor'
    if (category.includes('waybar') || category.includes('dunst') || category.includes('tofi') || category.includes('yazi')) return 'desktop'
    if (category.includes('home') || category.includes('etc') || category.includes('fastfetch')) return 'system'
          ...
          {#if node.expanded && node.children}
            <ul class="tree-sublist">
              {#each node.children as childNode (childNode.path)}
                <svelte:self tree={[childNode]} {activePath} {showOnlyAnnotated} />
              {/each}
            </ul>
          {/if}
        {:else}
          ...
          >
            ...
            <span class="node-name">{node.name}</span>
            {#if node.isAnnotated}
              <span class="annotation-indicator" style="background: {color}">①</span>
            {/if}
          </div>
        {/if}
      </li>
