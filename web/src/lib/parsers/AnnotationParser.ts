...
export class AnnotationParser {
	...
	parseAnnotations(file: string, content: string): Annotation[] {
		...
		const pattern = this.annotationPatterns[language as keyof typeof this.annotationPatterns]
			|| this.annotationPatterns.default
	...
	getCategory(filePath: string): string {
		...
		if (parts.includes('waybar') || parts.includes('dunst') || parts.includes('tofi') || parts.includes('yazi')) {
			return 'desktop'
		}
	...
	scanDirectory(
		dir: string,
		repoRoot: string,
		excludePatterns: string[] = ['.git', 'node_modules', '__gen', '.opencode'],
	): ConfigFile[] {
		...
		const scan = (currentDir: string) => {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory()) {
					...
					if (
						validExtensions.includes(ext) || name === 'makepkg.conf' || name.includes('kitty') || name.includes('hypr')
					) {
						try {
