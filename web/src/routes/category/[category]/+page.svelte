<script lang="ts">
  ...
  $: filteredFiles = data.files
    .filter((f) => {
      ...
      if (searchTerm && !f.name.toLowerCase().includes(searchTerm.toLowerCase()) && !f.path.toLowerCase().includes(searchTerm.toLowerCase())) {
        return false
      }
      return true
    })
...
<div class="category-page">
  <header class="page-header" style="border-color: {getCategoryColor(data.category)}">
    <div class="header-content">
      <a href="/" class="back-link">← Back to all files</a>
      <h1 class="category-title">{data.category}</h1>
      <p class="category-description">
        {filteredFiles.length} {filteredFiles.length === 1 ? 'file' : 'files'} in this category
      </p>
    </div>
  </header>
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-section">
        ...
        <div class="stats-grid">
          ...
          <div class="stat-item">
            <span class="stat-value">{data.files.filter((f) => f.isAnnotated).length}</span>
